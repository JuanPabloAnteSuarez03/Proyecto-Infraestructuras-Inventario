...
from app.repositories import SolicitudPiezaRepository
from app.models import SolicitudPieza
...
class SolicitudesPiezaService:
    def __init__(self, session: Session, repository: SolicitudPiezaRepository | None = None) -> None:
        ...
        # En este servicio no hay lógica adicional: CRUD plano

    def list(self) -> list[SolicitudPieza]:
        return self.repository.get_all()

    def delete_all(self) -> int:
        deleted = self.session.query(SolicitudPieza).delete()
        self.session.commit()
        return deleted
